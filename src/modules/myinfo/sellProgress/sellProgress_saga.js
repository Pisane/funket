...
import {
    SELL_PROGRESS_REQUESTING,
    sellProgressSuccess,
} from './sellProgress_store';

import axios from 'axios';
import * as config from '../../../config';
//const loginUrl = `http://localhost:7900/api/users/signin`;

async function getSellProgressApi(jwt, seq, lang)
{
    ...
    try{
        ...
        */
        ...
        const response = await axios({
            method: 'post',
            url: `${config.URL}/orders/mine/sellHistory/:seq`,
...
function* sellProgressFlow (jwt, seq, lang) {
    let response;
    try {
        response = yield call (getSellProgressApi, jwt, seq, lang);
        yield put(sellProgressSuccess(response));
    }catch (error) {
...
function* sellProgressWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(SELL_PROGRESS_REQUESTING);
        ...
        const task = yield fork(sellProgressFlow, jwt, seq, lang);
...
export default sellProgressWatcher;
