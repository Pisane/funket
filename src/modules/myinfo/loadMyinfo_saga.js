...
function* MyInfoFlow (jwt) {
    let response;
    try {
        response = yield call (myinfoApi, jwt);
...
function* myinfoWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(MYINFO_REQUESTING);
        ...
        const task = yield fork(MyInfoFlow, jwt);
