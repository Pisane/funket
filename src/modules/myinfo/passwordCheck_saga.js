...
import {
    PASSWORD_CHECK_REQUESTING,
    passwordCheckResult,
} from './MyInfoMng_store';
...
async function passwordCheckApi(jwt, pw)
{
...
function* passwordCheckFlow (jwt, pw) {
    let response;
    try {
        response = yield call (passwordCheckApi, jwt, pw);
        yield put(passwordCheckResult(response));
    }catch (error) {
...
function* passwordCheckWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(PASSWORD_CHECK_REQUESTING);
        ...
        const task = yield fork(passwordCheckFlow, jwt, pw);
...
export default passwordCheckWatcher;
