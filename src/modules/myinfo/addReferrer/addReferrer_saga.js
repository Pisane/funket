...
import {
    ADD_REFERRER_REQUESTING,
    addReferrerResult,
} from './addReferrer_store';
...
async function addReferrerApi(jwt, id)
{
    ...
    try{
        ...
        */
        ...
        const response = await axios({
            method: 'post',
            url: `${config.URL}/users/recomUserAdd`,
...
function* addReferrerFlow (jwt, id) {
    let response;
    try {
        response = yield call (addReferrerApi,jwt, id );
        yield put(addReferrerResult(response));
...
function* addReferrerWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(ADD_REFERRER_REQUESTING);
        ...
        const task = yield fork(addReferrerFlow, jwt, id);
...
export default addReferrerWatcher;
