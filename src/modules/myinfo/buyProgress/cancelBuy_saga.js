...
async function cancelBuyApi(seq, jwt)
{
    ...
    try{
        ...
        */
        ...
        const response = await axios({
            method: 'delete',
            url: `${config.URL}/orders/cancel/${seq}`,
...
function* cancelBuyFlow (seq, jwt) {
    let response;
    try {
        response = yield call (cancelBuyApi, seq, jwt);
        yield put(cancelBuySuccess(response));
    }catch (error) {
...
function* cancelBuyWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(CANCEL_BUY_REQUESTING);
        ...
        const task = yield fork(cancelBuyFlow, seq, jwt);
...
export default cancelBuyWatcher;
