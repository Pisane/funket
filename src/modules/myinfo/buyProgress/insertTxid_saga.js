...
async function insertTxidApi(seq, jwt, txid )
{
    ...
    try{
        ...
        */
        ...
        const response = await axios({
            method: 'put',
            url: `${config.URL}/orders/txid/${seq}`,
...
function* insertTxidFlow (seq, jwt, txid) {
    let response;
    try {
        response = yield call (insertTxidApi, seq, jwt , txid);
        yield put(insertTxidResult(response));
...
function* insertTxidWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(INSERT_TXID_REQUESTING);
        ...
        const task = yield fork(insertTxidFlow, seq, jwt, txid);
...
export default insertTxidWatcher;
