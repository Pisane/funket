...
async function passwordChangeApi(jwt,pw)
{
...
function* passwordChangeFlow (jwt,pw) {
    let response;
    try {
        response = yield call (passwordChangeApi, jwt, pw);
        yield put(passwordChangeResult(response));
    }catch (error) {
        response = {result : 2};
...
function* passwordChangeWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(PASSWORD_CHAGNE_REQUESTING);
        ...
        const task = yield fork(passwordChangeFlow, jwt, pw);
...
export default passwordChangeWatcher;
