...
import {
    USERINFO_DETAIL_REQUESTING,
    userInfoDetailResult,
} from './passwordChange_store';
...
async function userinfoDetailApi(jwt)
{
    ...
    try{
        ...
        */
        ...
        const response = await axios({
            method: 'get',
            url: `${config.URL}/users/detailInfo`,
        ...
        //     id: 'SIR1234',
        //     name: '홍길동',
        //     email: 'rlfehd@naver.com',
        //     wallet: 'rkskekfkaksjkl',
        // }
...
function* userinfoDetailFlow (jwt) {
    let response;
    try {
        response = yield call (userinfoDetailApi, jwt);
        yield put(userInfoDetailResult(response));
    }catch (error) {
...
function* userinfoDetailWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(USERINFO_DETAIL_REQUESTING);
        ...
        const task = yield fork(userinfoDetailFlow, jwt);
...
export default userinfoDetailWatcher;
