...
async function getTradeHistoryDetailApi(jwt, seq, type, lang)
{
...
//         const response = data1;
        ...
        const response = await axios({
            method: 'post',
            url: `${config.URL}/orders/mine/tradeHistory/:seq/:type`,
...
function* tradeHistoryDetailFlow (jwt, seq, type, lang) {
    let response;
    try {
        response = yield call (getTradeHistoryDetailApi, jwt, seq, type, lang);
...
function* tradeHistoryDetailWatcher () { 
    ...
    while( true ){
        ...
        const task = yield fork(tradeHistoryDetailFlow, jwt, seq, type, lang);
