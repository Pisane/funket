...
async function getBuyTradingApi(jwt, lang)
{
    ...
    try{
        ...
        */
        ...
        const response = await axios({
            method: 'post',
            url: `${config.URL}/orders/mine/buyHistory`,
...
function* buyTradingFlow (jwt, lang) {
    let response;
    try {
        //console.log('buyTradingFlow')
        response = yield call (getBuyTradingApi, jwt, lang);
        //console.log(response);
        yield put(buyTradingSuccess(response));
    }catch (error) {
...
function* buyTradingWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(BUY_TRADING_REQUESTING);
        ...
        const task = yield fork(buyTradingFlow, jwt, lang);
...
export default buyTradingWatcher;
