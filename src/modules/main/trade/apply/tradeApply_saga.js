...
import {
    TRADE_APPLY_REQUESTING,
    tradeApplyResult,
} from './tradeApply_store';
...
async function applyTradeApi(jwt,seq,count, lang)
{
    ...
    try{
       ...
       */
        ...
        const response = await axios({
            method: 'post',
            url: `${config.URL}/orders/purchase/${seq}`,
...
function* applyTradeFlow (jwt, seq, count, lang) {
    let response;
    try {
        response = yield call (applyTradeApi, jwt, seq, count, lang);
        yield put(tradeApplyResult(response));
    }catch (error) {
...
function* applyTradeWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(TRADE_APPLY_REQUESTING);
        ...
        const task = yield fork(applyTradeFlow, jwt, seq, count, lang);
...
export default applyTradeWatcher;
