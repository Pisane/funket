...
import {
    MILEAGE_HOME_REQUESTING,
    mileageHomeSuccess,
} from './mileageHome_store';
...
async function getMileageHomeApi(jwt, lang)
{
    ...
    //    type: null,
        ...
        const response = await axios({
            method: 'post',
            url: `${config.URL}/mileages/mylist`,
...
function* mileageHomeFlow (jwt, lang) {
    let response;
    try {
        response = yield call (getMileageHomeApi, jwt, lang);
        yield put(mileageHomeSuccess(response));
    }catch (error) {
...
function* mileageHomeWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(MILEAGE_HOME_REQUESTING);
        ...
        const task = yield fork(mileageHomeFlow, jwt, lang);
...
export default mileageHomeWatcher;
