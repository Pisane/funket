...
import {
    MILEAGE_CHARGE_REQUESTING,
    mileageChargeResult,
} from './mileageCharge_store';
...
async function mileageChargeApi(jwt, value)
{
    ...
    try{
       ...
       */
        ...
        // const response = {chargeResult: 1}; // 1: success or 2: fail
        const headerParams = {
        ...
        const response = await axios({
            method: 'post',
            url: `${config.URL}/mileages/charge`,
...
function* mileageChargeFlow (jwt, value) {
    let response;
    try {
        response = yield call (mileageChargeApi, jwt, value);
        
        yield put(mileageChargeResult(response));
    }catch (error) {
        // error? send it to redux
        
        response = {chargeResult: 2}
        yield put(mileageChargeResult(response));
...
function* mileageChargeWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(MILEAGE_CHARGE_REQUESTING);
        ...
        const task = yield fork(mileageChargeFlow, jwt, value);
...
export default mileageChargeWatcher;
