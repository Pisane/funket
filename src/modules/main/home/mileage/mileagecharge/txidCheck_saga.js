...
import {
    TXID_CHECK_REQUESTING,
    txidCheckResult,
} from './txidCheck_store';
...
async function checkTxidApi(jwt, seq, txid)
{
    ...
    try{
       ...
       */
        ...
        // const response = {
        //     txidCheckResult : 1, // ok : 1 or Fail : 2
        // };
        const headerParams = {
        ...
        const response = await axios({
            method: 'put',
            url: `${config.URL}/mileages/chargeTxid/${seq}`,
...
function* txidCheckFlow (jwt, seq, txid) {
    let response;
    try {
        response = yield call (checkTxidApi, jwt, seq, txid);
     
        yield put(txidCheckResult(response));
    }catch (error) {
        // error? send it to redux
        // yield put(errorIn(error));
        // console.log('error')
        response = {txidCheckResult: 2}
        yield put(txidCheckResult(response));
...
function* txidCheckWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(TXID_CHECK_REQUESTING);
        ...
        const task = yield fork(txidCheckFlow, jwt, seq, txid);
...
export default txidCheckWatcher;
