...
async function findPWApi(id, email)
{
...
function* findPWFlow (id,email) {
    let response;
    try {
        response = yield call (findPWApi, id, email);
...
function* findPWWatcher () { 
    ...
    while( true ){
        ...
        const task = yield fork(findPWFlow, id, email);
